...
(function (i, n) {
    ...
    a.measureContainment = function () {
        ...
        var a = (this.relativeStartPosition = {
...
export default class TabRowWidget extends BasicWidget {
            ...
            contextMenu.show({
                ...
                items: [
                    ...
                    },
                    {
                        ...
                        command: "removeAllTabsExceptForThis",
                        uiIcon: "x",
                    },
                ],
    ...
    get tabWidths() {
        ...
        for (let i = 0; i < numberOfTabs; i += 1) {
            const extraWidth =
                flooredClampedTargetWidth < TAB_CONTAINER_MAX_WIDTH &&
        ...
        if (this.$filler) {
            this.$filler.css(
                "width",
                extraWidthRemaining + MIN_FILLER_WIDTH + "px"
            );
        }
    ...
    layoutTabs() {
        ...
        const { tabPositions, newTabPosition, fillerPosition } =
            this.getTabPositions();

        tabPositions.forEach((position, i) => {
            styleHTML += `.note-tab:nth-child(${
                i + 1
    ...
    cleanUpPreviouslyDraggedTabs() {
        this.tabEls.forEach((tabEl) =>
            tabEl.classList.remove("note-tab-was-just-dragged")
        );
    }
    ...
    setupDraggabilly() {
        ...
        tabEls.forEach((tabEl, originalIndex) => {
            ...
            draggabilly.on("pointerDown", (_) => {
                appContext.tabManager.activateTab(
            ...
            draggabilly.on("dragMove", (event, pointer, moveVector) => {
                ...
                if (Math.abs(moveVector.y) > 100) {
                    this.triggerCommand("moveTabToNewWindow", {
                        tabId: this.getTabId($(tabEl)),
                    });
                }
            });
        });
    ...
    animateTabMove(tabEl, originIndex, destinationIndex) {
        if (destinationIndex < originIndex) {
            ...
            const beforeEl =
                this.tabEls[destinationIndex + 1] || this.$newTab[0];

            tabEl.parentNode.insertBefore(tabEl, beforeEl);
        }
        this.triggerEvent("tabReorder", {
            tabIdsInOrder: this.getTabIdsInOrder(),
        });
        this.layoutTabs();
    }
    ...
    updateTab($tab, note) {
        ...
        const tabContext = appContext.tabManager.getTabContextById(
            this.getTabId($tab)
        );

        if (tabContext) {
            const hoistedNote = treeCache.getNoteFromCache(
                tabContext.hoistedNoteId
            );
            ...
            if (hoistedNote) {
                ...
                $tab.find(".note-tab-wrapper").css(
                    "background",
                    hoistedNote.getWorkspaceTabBackgroundColor()
                );
            } else {
    ...
    async entitiesReloadedEvent({ loadResults }) {
        for (const tabContext of appContext.tabManager.tabContexts) {
            ...
            if (
                loadResults.isNoteReloaded(tabContext.noteId) ||
                loadResults
                    .getAttributes()
                    .find(
                        (attr) =>
