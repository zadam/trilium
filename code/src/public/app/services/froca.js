...
class Froca {
    ...
    getSwimlaneTasks(swimlaneId) {
        const result = [];
        const attrs = Object.values(this.attributes);
        for (const i in attrs) {
            const a = attrs[i];
            if (a.type === 'taskprop' && a.name === 'swimlane' && a.value === swimlaneId) {
                result.push(this.notes[attrs[i].noteId]);
            }
        }
        return result;
    }
    ...
    async getParentNotes(noteId) {
        const note = await this.getNote(noteId);
        return note.parents;
    }

    async getAttributes(noteId) {
        const n = await this.getNote(noteId);
        return n.attributes.map((attrId) => this.attributes[attrId]);
    }
    ...
    async createAttribute(noteId, attribute) {

        await server.put(`notes/${noteId}/set-attribute`, attribute);
    }

    async isParentOf(p, note) {
        ...
        for (const i in parents) {
            const parent = await this.getNote(parents[i]);
            const res = await this.isParentOf(p, parent);
            if (res) {
                return true;
            }
        }
    ...
    async getParentsList(note) {
        const res = [note];
        var parents = note.parents;
        while (parents && parents.length > 0) {
            var n = await this.getNote(parents[0]);
            res.unshift(n);
            parents = n.parents;
        }
        return res;
