...
async function renderAttribute(attribute, renderIsInheritable) {
    ...
    } else if (attribute.type === 'taskprop') {
        $attr.append(`name: ${attribute.name}, value: ${attribute.value}, type: ${attribute.type}`)
    } else if (attribute.type === 'dashboardprop' || attribute.type == 'swimlaneprop') {
        return "";
    }else {
...
async function renderAttributes(attributes, renderIsInheritable) {
    ...
    for (let i = 0; i < attributes.length; i++) {
        ...
        if (attribute.type === 'taskprop' || attribute.type === 'swimlaneprop' || attribute.type === 'dashboardprop') {
            continue;
        }
