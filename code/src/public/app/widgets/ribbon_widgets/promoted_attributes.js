...
export default class PromotedAttributesWidget extends NoteContextAwareWidget {
    ...
    async createPromotedAttributeCell(definitionAttr, valueAttr, valueName) {
        ...
        const $input = definition.labelType === 'enum' ? $("<label>") : $("<input>")
        ...
        if (valueAttr.type === 'label') {
            if (definition.labelType === 'enum') {
                let item = "<select>";
                definition.enumValues.forEach(function(str, index){
                    item = item + "<option value=\""+str+"\">" + str + "</option>";
                });
                item = item + "</select>";
                $input.after(item);
            }
            else if (definition.labelType === 'text') {
