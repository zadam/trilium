...
export default class NoteTreeWidget extends NoteContextAwareWidget {
    ...
    doRender() {
        ...
        this.$tree.on("mousedown", ".mark-task-done", async e => {
            const node = $.ui.fancytree.getNode(e);
    ...
    async taskStatusUpdatedEvent(e) {
        ...
        const traverse = (node, noteId) => {
            if (node.data.noteId === noteId) {
                return node;
            } else {
                for(const i in node.children){
                    const res = traverse(node.children[i], noteId);
                    if (res) {
                        return res;
                    }
                } 
        ...
        const node = traverse(this.tree.getRootNode().children[0], noteId);
        if (node) {
            await this.updateNode(node);
        } else {
            console.log('task this ID not found');
        }
    }
        ...
        this.prepareRootNode().then( d => {
            ...
            this.$tree.fancytree({
                ...
                enhanceTitle: async function (event, data) {
                    ...
                    $span.find('.tree-item-tag').remove();
                    const $span = $(node.span);
                    ...
                    const status = note.attributes.map(a => froca.attributes[a]).find(a => a.name === 'state')?.value ?? 'N/A';

                    if (note.type === 'task') {

                        if (status !== 'done') {
                            const $markDoneButton = $('<span class="tree-item-button mark-task-done bx bx-check" title="Mark this task as done"></span>'); 
                            $span.append($markDoneButton);
                        } 
                    }

                    if (note.type !== 'swimlane' && !['search', 'launcher'].includes(note.type) && !note.isOptions() && !note.isLaunchBarConfig()) {
                    ...
                    if (note.type === 'task') {
                        ...
                        const createTag = (text, color) => {
                            const tag = $(`<span class="tree-item-tag task-done" title="${text}">${text}</span>`); 
                            tag.css('border-color', color);
                            $span.append(tag);
                        };
                        if (status === 'N/A') {
                            throw new Error('N/A is not a valid value for task status: ' + note.title);
                        }
                        else if (status === 'done'){
                            createTag('done', '#00ff00');
                        } else {
                            if (status === 'blocked') {
                                createTag('blocked', 'orange');
                            } else if (status === 'postponed') {
                                createTag('postponed', 'yellow');
                            } else if (status === 'default') {
                                createTag('not started', 'gray')
                            }
                        }
                    }
                },
    ...
    async prepareRootNode() {
        return await this.prepareNode(froca.getBranch('none_root'));
    async prepareRootNode() {
        return await this.prepareNode(froca.getBranch('none_root'));
    }
     ...
     */
    async prepareChildren(parentNote) {
    async prepareChildren(parentNote) {
        utils.assertArguments(parentNote);

        const noteList = [];

        const hideDoneTasks = this.hideDoneTasks;
        const hideDoneTasks = this.hideDoneTasks;
        ...
        for (const branch of childBranches) {
            ...
            if (hideDoneTasks) {
                ...
                if (await taskService.isDone(note)) {
                    continue;
                }
            }
     ...
     */
    async prepareNode(branch, forceLazy = false) {
    async prepareNode(branch, forceLazy = false) {
        const note = branch.getNoteFromCache();
            ...
            if (!branchRow.isDeleted) {
                for (const parentNode of this.getNodesByNoteId(branchRow.parentNoteId)) {
                    ...
                    if (!found) {
                        ...
                        parentNode.addChildren([await this.prepareNode(frocaBranch, true)]);
                        parentNode.addChildren([await this.prepareNode(frocaBranch, true)]);
