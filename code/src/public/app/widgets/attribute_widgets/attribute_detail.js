        ...
        <tr class="attr-row-label-type">
            ...
            <td>
                <select class="attr-input-label-type form-control">
                  ...
                  <option value="enum">Custom Enum</option>
                  <option value="url">URL</option>
                </select>
            </td>
        </tr>
        <tr class="attr-row-enum">
            <th>Custom Enum Values:</th>
            <td>
                <div class="input-group">
                    <input type="text" class="form-control attr-input-enum"/>
                    <p>*List of comma-separated enum values</p>
                </div>
            </td>
        </tr>
...
export default class AttributeDetailWidget extends NoteContextAwareWidget {
    ...
    doRender() {
        ...
        this.$rowEnumValue = this.$widget.find('.attr-row-enum');
        this.$rowEnumValueInput = this.$widget.find('.attr-input-enum');
        this.$rowEnumValueInput.on('change', () => this.userEditedAttribute());
    ...
    async showAttributeDetail({allAttributes, attribute, isOwned, x, y, focus}) {
        ...
        this.$rowEnumValue.toggle(this.attrType === 'label-definition' && definition.labelType === 'enum');
        if (definition.labelType === 'enum') {
            
        }
        this.$rowEnumValueInput
            .val(definition.enumValue)
            .attr('disabled', () => !isOwned);
    ...
    buildDefinitionValue() {
        ...
        if (this.attrType === 'label-definition') {
            ...
            } else if (this.$inputLabelType.val() === 'enum' && this.$rowEnumValueInput.val() !== '') {
                props.push(`enumValues=${this.$rowEnumValueInput.val()}`);
            }
