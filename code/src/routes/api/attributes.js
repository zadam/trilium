...
function updateNoteAttribute(req) {
    ...
    if (attribute.type === 'label' || attribute.type === 'taskprop' || body.value.trim()) {
...
function addNoteAttribute(req) {
    ...
    return sql.getValue(`SELECT attributeId FROM attributes WHERE isDeleted = 0 AND noteId = ? AND type = ? AND name = ?`, [noteId, body.type, body.name]);
}
